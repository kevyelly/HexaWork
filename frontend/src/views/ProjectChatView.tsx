...
interface ChatHistory { roomId: string; walletAddress: string; name: string; lastMessage: string; time: string; timestamp: number; jobTitle?: string; hasNotification?: boolean; }
    ...
    useEffect(() => {
        ...
        const fetchHistory = async () => {
            ...
            if (isAdminUser) {
                ...
                // For Admins, red dot implies a pending dispute resolution in that room
                ...
                if (uniqueWallets.length > 0) {
                    ...
                    if (usersData) {
                        usersData.forEach(user => {
                            historyMap.forEach(chat => { if (chat.walletAddress === user.wallet_address.toLowerCase()) { chat.name = user.full_name || formatAddress(user.wallet_address); } });
                        });
                    }
                }
                            ...
                            {userRole === 'employer' && activeMilestone.status === 'submitted' && (
                                <div className="border-t border-zinc-100 pt-6">
                                    ...
                                    {!aiVerdict && !activeMilestone.ai_verdict ? (
                                        ...
                                        <div className={`p-5 rounded-2xl border ${isAiApproved(aiVerdict?.status) || isAiApproved(activeMilestone.ai_verdict) ? 'bg-emerald-50 border-emerald-200' : 'bg-amber-50 border-amber-200'}`}><div className="flex items-center gap-2 mb-2"><span className={`text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded-md ${isAiApproved(aiVerdict?.status) || isAiApproved(activeMilestone.ai_verdict) ? 'bg-emerald-200 text-emerald-800' : 'bg-amber-200 text-amber-800'}`}>Recommendation: {isAiApproved(aiVerdict?.status) || isAiApproved(activeMilestone.ai_verdict) ? 'APPROVE' : 'DISPUTE'}</span></div><p className="text-sm font-medium text-zinc-800 leading-relaxed mb-4">{aiVerdict?.reasoning || "AI evaluation complete."}</p></div>
                                    )}
                                </div>
                            )}
                            ...
                            {milestones.map((m, index) => {
                                ...
                                return (
                                    <div key={m.id} className={`p-4 border rounded-2xl transition-all ${m.status === 'approved' ? 'bg-emerald-50 border-emerald-100' : isAttentionNeeded && !isContractCancelled && !isProjectCompleted ? 'bg-amber-50 border-amber-300 shadow-md animate-pulse' : 'bg-white border-zinc-200'}`}>
                                        ...
                                        {/* Buttons exclusively handle Resubmit or Opening the new Appeal modal */}
